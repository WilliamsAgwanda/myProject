  ...
  <center><section class="sec1">
   ...
   <form class="form" enctype="multipart/form-data" method="POST" action=<?php $_SERVER["PHP_SELF"];?>><br>
          ...
          <br><br>
           ...
           <div>
             <input class="log" type="submit" value="DISCHARGE" name="discharge">
       ...
       <?php
          if(isset($_POST['discharge'])){
            ...
            $state="Discharged";
            $admit="Admitted";
            $sq="SELECT regno from patients where regno='$reg'";
            $rq=mysqli_query($conn,$sq);
            if(mysqli_num_rows($rq)==1){
         if(preg_match("/^[0-9-a-zA-Z\/]*$/",$reg)){

            $sql="UPDATE admit set statuss='$state',dod=curdate() where regno='$reg' && statuss='$admit';";
            $r=mysqli_query($conn,$sql);
            if($r){
              echo'<script type="text/javascript">alert("Discharged");
              window.location.replace("doctor.php");</script>';
            }else{
          ...
          echo"<script>alert('Please Check Regno And Try Again')</script>";
