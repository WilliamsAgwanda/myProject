...
<body>
  ...
  <center><section class="sec1">
   ...
   <form class="form" enctype="multipart/form-data" method="POST" action=<?php $_SERVER["PHP_SELF"];?>><br>
          ...
          <i><b><h2>Ailment Records</h2></b></i><br><br>
           <div class="form-group toleft">
             <h5>Ailment:</h5>
             <input type="text" required name="ailment" >
           </div>
           <br>
      ...
      <?php
          if(isset($_POST['add'])){
            ...
            if(preg_match("/^[a-zA-Z-' ]*$/",$ailment)){
            $sql="INSERT into mrt(regno,ailment,dat)values('$reg','$ailment',curdate());";
            $r=mysqli_query($conn,$sql);
            if($r){
              echo'<script type="text/javascript">alert("Added successfully");
              window.location.replace("docpat.php");</script>';
            }else{
