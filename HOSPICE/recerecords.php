...
if(isset($_POST['ok'])){
  $idno=$_POST['idno'];
  $sq="SELECT idno from patients where idno='$idno'";
            $rq=mysqli_query($conn,$sq);
            if(mysqli_num_rows($rq)==1){
    $_SESSION['idno']=$idno;
   if(preg_match("/^[0-9]*$/",$idno)){

    header('location:patrec.php');
   }else{
    echo"<script>alert('Incorrect Format')</script>";

   }
  }else{
    echo"<script>alert('Please Check Id No And Try Again')</script>";
   ...
   <div class="container">
      <div class="profile">
        ...
        <form class="form" enctype="multipart/form-data" method="POST" action=<?php $_SERVER["PHP_SELF"];?>><br><br><br>
          ...
          <i><b><h2>Enter ID No</h2></b></i><br><br>
           <div class="form-group toleft">
             <h5> ID No:</h5>
             <input type="text" required name="idno" >
           </div>
           <br><br>
