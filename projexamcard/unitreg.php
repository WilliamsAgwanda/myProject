        ...
        <div class="row card">
         ...
         $sql = "SELECT * FROM students where email='$user' && regno='$pass';";
          ...
          <div class="card card-success">
            ...
            <?php     
                  $sql = "SELECT stage from students where email='$user' && regno='$pass';";
                    $result= mysqli_query($conn,$sql);
                  while($data=mysqli_fetch_array($result)){
                    $st=$data['stage'];
                  }
            ...
            ?>
                      <div class="icheck form-group">
                        <input type="checkbox" name="choosen[]" value="<?php echo $code." ".$unit; ?>" >
                        <label><?php echo $code." ".$unit; ?>
                        </label>
                      </div>
                      <?php } ?>
                  ...
                  <?php
                       if(isset($_POST['submit'])){
                         for($i=0;$i<sizeof($_POST['choosen']);$i++){
                            $sql3="CREATE table $pass(unit varchar(100) not null primary key,grade varchar(100));"; 
                              mysqli_query($conn,$sql3);
                              $s=$_POST['choosen'][$i];
                            $sql4="INSERT into $pass(unit,grade)values('$s','');"; 
                            mysqli_query($conn,$sql4);
                         }

                         $sql5="SELECT * from $pass;"; 

                  if (mysqli_num_rows(mysqli_query($conn, $sql5)) == 0) {
                             echo '<script>alert("TRY AGAIN")</script>';
                    
                }elseif (mysqli_num_rows(mysqli_query($conn, $sql5))>=1) {
                  echo '<script>alert("units registration was successful")</script>';
