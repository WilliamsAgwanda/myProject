...
<?php
            ...
            if(preg_match("/^[0-9]*$/",$pin)){
               ...
               $sql="SELECT pin from customer where idno='$pass';";
               $r=mysqli_query($conn,$sql);
               while($d=mysqli_fetch_array($r)){
                 $p=$d['pin'];
                 if($p===$pin){
                   header('location:confwithd.php');
                 }else{
                   echo"<script>alert('Incorrect pin')</script>";
                 }
               }
               
             //   $sql1="SELECT credit from customer where idno='$pass';";
             // $r=mysqli_query($conn,$sql);
             // while($d=mysqli_fetch_array($r)){
             //   $credit=$d['credit'];
               
             //   $sql2="UPDATE customer set credit=$credit-$amount where idno='$pass';";
   ...
   <div class="container">
      <div class="profile">
        <nav class="navbar navbar-light bg-light">
            <a href="profile.php"><img src="arrow.jpg"></a>
        ...
        <form class="form" method="POST" action=<?php $_SERVER["PHP_SELF"];?>><br><br><br>
           ...
           <div class="form-group toleft">
             <h5>Enter Pin:</h5>
             <input type="number" required name="pinf">
           </div>
