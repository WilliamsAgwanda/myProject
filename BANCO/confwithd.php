...
<?php
       ...
       if(isset($_POST['ok'])){
        $sql4="SELECT credit,accountno from customer where idno='$pass';";
        $r4=mysqli_query($conn,$sql4);
        $sql5="INSERT into stat(idno,withdraw,dat)values('$pass',$amount,curdate());";
        $r5=mysqli_query($conn,$sql5);
        while($d4=mysqli_fetch_array($r4)){
          $credit=$d4['credit'];
          $acccountno=$d4['accountno'];
          $newcredit=$credit-$amount;
                 
           $sql2="UPDATE customer set credit=$newcredit where idno='$pass';";
           $r2=mysqli_query($conn,$sql2);
           $sql3="SELECT credit from customer where idno='$pass';";
  ...
  <center><section class="sec1">
   ...
   <div class="container">
      <div class="profile">
        ...
        <form class="form" method="POST" action=<?php $_SERVER["PHP_SELF"];?>><br><br><br>
          
           <div class="form-group toleft">
             <?php
                $sql="SELECT credit,accountno from customer where idno='$pass';";
                $r=mysqli_query($conn,$sql);
                while($d=mysqli_fetch_array($r)){
                  $credit=$d['credit'];
                  $acccountno=$d['accountno'];
                  $newcredit=$credit-$amount;
             ?>
             <h5>Withdraw <?php echo $amount.".00"; ?> from Account No: <?php echo $acccountno; ?> to your M-Pesa</h5>
             </div><br><br><?php } ?>
