...
$pass=$_SESSION['pass'];
$op=$_SESSION['oldpin'];
$newpin=$_SESSION['newpin'];
?>
  <?php
   if(isset($_POST['ok'])){
     $confpin=$_POST['confpin'];
     if(preg_match("/^[0-9]*$/",$confpin)){

     if($confpin==$newpin){
       $sql="call changepin('$confpin','$pass','$op');";
       $r=mysqli_query($conn,$sql);
       $sql1="SELECT pin from customer where  idno='$pass' && pin='$confpin';";
       $r1=mysqli_query($conn,$sql1);

       if(mysqli_num_rows($r1)===1){
         header("location:pinsuccess.php");
       }else{
         echo"<script>alert('pin numbers do not match try again')</script>";
       }
     }else{
      echo"<script>alert('pin numbers do not match try again')</script>";
    }
   ...
   <div class="container">
      <div class="profile">
        <nav class="navbar navbar-light bg-light">
            <a href="setpin.php"><img src="arrow.jpg"></a>
            <a><img src="menu.jpg"></a>
        </nav>
        <form class="form" method="POST" action=<?php $_SERVER["PHP_SELF"];?>><br><br><br>
          
           <div class="form-group toleft">
             <h5>Comfirm New Pin:</h5>
             <input type="number" required name="confpin">
           </div>
           
           <div>
             <input class="log" type="submit" name="ok" value="OK">
