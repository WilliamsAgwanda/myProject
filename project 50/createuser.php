      ...
      <div class="container-fluid">
        ...
        <div class="row">
              ...
              <form action="<?php $_SERVER["PHP_SELF"];?>" method="POST" class="form-group">
                ...
                <div class="form-group">
                  Host:
                  <input type="text" name="hst" class="form-control"placeholder="Enter host">
                </div>
                <div class="form-group">
                  Password:
                  <input type="text" name="psw" class="form-control"placeholder="Enter password">
                </div>
                <div class="form-group">
...
<?php
 if(isset($_POST['submit'])){
  if($user=='root' && $h=="localhost"){
     ...
     $hst=$_POST['hst'];
     $psw=$_POST['psw'];
  if(!empty($usr) && !empty($hst) && !empty($psw) ){

     $c1="create user '$usr'@'$hst' identified by '$psw';";
     mysqli_query($conn,$c1);
     $c21="INSERT into upt(user,psw,host)values('$usr','$psw','$hst');";
     mysqli_query($conn,$c21);
     $c23="SELECT user from upt where user='$usr' && psw='$psw' && host='$hst';";
     mysqli_query($conn,$c23);
     if (mysqli_num_rows(mysqli_query($conn,$c23))==1) {
        echo '<script>alert("User added successfully")</script>';
        exit();
      }elseif(mysqli_num_rows(mysqli_query($conn,$c23))==0){
      
       echo '<script>alert("A Problem was Detected")</script>';
        
 
     }
    }else {  echo '<script>alert("Fill all the details")</script>';}
  }else {  echo '<script>alert("You are not allowed to create a user")</script>';}
 }
?>
